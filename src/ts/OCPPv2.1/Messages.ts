...
export interface ClearChargingProfileRequest {
    ...
    chargingProfileCriteria?:           complex.ClearChargingProfile,           // Specifies the charging profile.
    customData?:                        complex.ICustomData                     // Customer specific data.
}
...
export interface GetInstalledCertificateIdsResponse {
    ...
    certificateHashDataChain?:          Array<complex.CertificateHashDataChain> // The Charging Station includes the Certificate information for each available certificate.
    statusInfo?:                        complex.StatusInfo,                     // Detailed status information.
    customData?:                        complex.ICustomData                     // Customer specific data.
}
...
export interface NotifySettlementRequest {
    ...
    vatCompany?:                        complex.Address,                            // Company address associated with VAT number.
    customData?:                        complex.ICustomData                         // Customer specific data.
}
...
export interface SetVariablesRequest {
    setVariableData:                    Array<complex.SetVariableData>,             // List of Component-Variable pairs and attribute values to set.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface SetVariablesResponse {
    setVariableResult:                  Array<complex.SetVariableResult>,           // List of result statuses per Component-Variable.
    customData?:                        complex.ICustomData                         // Customer specific data.
}
...
export interface SignCertificateResponse {
    status:                             types.  GenericStatus,                      // Specifies whether the CSMS can process the request.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface StatusNotificationRequest {
    timestamp:                          types.  Timestamp,                          // The time for which the status is reported.
    connectorStatus:                    types.  ConnectorStatus,                    // This contains the current status of the Connector.
    evseId:                             types.  EVSEId,                             // The id of the EVSE to which the connector belongs for which the the status is reported.
    connectorId:                        types.  ConnectorId,                        // The id of the connector within the EVSE for which the status is reported.
    customData?:                        complex.ICustomData                         // Customer specific data.
}
...
export interface TransactionEventRequest {
    eventType:                          types.  TransactionEvent,                   // This contains the type of this event. The first TransactionEvent of a transaction
                                                                                    // SHALL contain: "Started"
                                                                                    // The last TransactionEvent of a transaction SHALL contain: "Ended"
                                                                                    // All others SHALL contain: "Updated"
    timestamp:                          types.  Timestamp,                          // The date and time at which this transaction event occurred.
    triggerReason:                      types.  TriggerReason,                      // Reason the Charging Station sends this message to the CSMS.
    seqNo:                              types.  Integer,                            // Incremental sequence number, helps with determining if all messages of a transaction
                                                                                    // have been received.
    offline?:                           boolean,                                    // Indication that this transaction event happened when the Charging Station was offline.
                                                                                    // Default = false, meaning: the event occurred when the Charging Station was online.
    numberOfPhasesUsed?:                types.  Integer,                            // If the Charging Station is able to report the number of phases used, then it SHALL
                                                                                    // provide it. When omitted the CSMS may be able to determine the number of phases used
                                                                                    // as follows:
                                                                                    //   1: The numberPhases in the currently used ChargingSchedule.
                                                                                    //   2: The number of phases provided via device management.
    cableMaxCurrent?:                   types.  Ampere,                           // The maximum current of the connected cable in Ampere (A).
    reservationId?:                     types.  ReservationId,                      // This contains the Id of the reservation that terminates as a result of this transaction.
    preconditioningStatus?:             types.  PreconditioningStatus,              // (2.1) The current preconditioning status of the BMS in the EV. Default value is Unknown.
    evseSleep?:                         boolean,                                    // (2.1) True when EVSE electronics are in sleep mode for this transaction.
                                                                                    //       Default value (when absent) is false.
    meterValue?:                        Array<complex.MeterValue>,                  // This contains the meter values relevant to the transaction. Depending on the eventType
                                                                                    // of this TransactionEventRequest the following Configuration Variable is used to
                                                                                    // configure which measurands are used:
                                                                                    //   Started: SampledDataTxStartedMeasurands
                                                                                    //   Updated: SampledDataTxUpdatedMeasurands and AlignedDataMeasurands
                                                                                    //   Ended:   SampledDataTxEndedMeasurands and AlignedDataTxEndedMeasurands
    idToken?:                           complex.IdToken,                            // This contains the identifier for which a transaction is (or will be) started or stopped.
                                                                                    // Is required when the EV Driver becomes authorized for this transaction and when the EV
                                                                                    // Driver ends authorization.
                                                                                    // The IdToken should only be sent once in a TransactionEventRequest for every authorization
                                                                                    // (for starting or for stopping) done for this transaction, so that CSMS can return the
                                                                                    // idTokenInfo in the TransactionEventResponse.
...
export interface TransactionEventResponse {
    totalCost?:                         types.  Decimal,                            // When eventType of TransactionEventRequest is Updated, then this value contains the
                                                                                    // running cost. When eventType of TransactionEventRequest is Ended, then this contains
                                                                                    // the final total cost of this transaction, including taxes, in the currency configured
                                                                                    // with the Configuration Variable: Currency. Absence of this value does not imply that
                                                                                    // the transaction was free. To indicate a free transaction, the CSMS SHALL send a value
                                                                                    // of 0.00.
    chargingPriority?:                  types.  Integer,                            // Priority from a business point of view. Default priority is 0.
                                                                                    // The range is from -9 to 9. Higher values indicate a higher priority.
                                                                                    // The chargingPriority in TransactionEventResponse is temporarily, so it may not be set
                                                                                    // in the IdTokenInfoType afterwards. Also the chargingPriority in TransactionEventResponse
                                                                                    // has a higher priority than the one in IdTokenInfoType.
    idTokenInfo?:                       complex.IdTokenInfo,                        // This contains information about authorization status, expiry and group id. Is required
                                                                                    // when the transactionEventRequest contained an idToken.
    updatedPersonalMessage?:            complex.MessageContent,                     // This can contain updated personal message that can be shown to the EV Driver.
                                                                                    // This can be used to provide updated tariff information.
    updatedPersonalMessageExtra?:       Array<complex.MessageInfo>,                 // (2.1) Additional languages besides the default language in updatedPersonalMessage.
    transactionLimit?:                  complex.TransactionLimit,                   // (2.1) Maximum cost/energy/time limit allowed for this transaction.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface TriggerMessageRequest {
    requestedMessage:                   types.MessageTrigger,                       // The message to be triggered.
    customTrigger?:                     string,                                     // (2.1) When requestedMessage = CustomTrigger this will trigger sending the corresponding
                                                                                    // message in field customTrigger, if supported by Charging Station.
    evse?:                              complex.EVSE,                               // Can be used to specifiy the EVSE and Connector if required for the message which needs
                                                                                    // to be sent.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface TriggerMessageResponse {
    status:                             types.  TriggerMessageStatus,               // This indicates whether the Charging Station is able to trigger the message.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UnlockConnectorRequest {
    evseId:                             types.  EVSEId,                             // The id of the EVSE for which the connector needs to be unlocked.
    connectorId:                        types.  ConnectorId,                        // The id of the connector that needs to be unlocked.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UnlockConnectorResponse {
    status:                             types.  UnlockStatus,                       // This indicates whether the Charging Station is able to unlock the connector.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UnpublishFirmwareRequest {
    checksum:                           string,                                     // The MD5 checksum over the entire firmware file as a hexadecimal string of
                                                                                    // length 32.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UnpublishFirmwareResponse {
    status:                             types.  UnpublishFirmwareStatus,            // Indicates whether the Local Controller succeeded in unpublishing the firmware.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UpdateDynamicScheduleRequest {
    chargingProfileId:                  types.  ChargingProfileId,                  // Id of charging profile to update.
    scheduleUpdate:                     complex.ChargingScheduleUpdate,             // Updated values for charging schedule period.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UpdateDynamicScheduleResponse {
    status:                             types.  ChargingProfileStatus,              // Returns whether message was processed successfully.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UpdateFirmwareRequest {
    retries?:                           types.  Integer,                            // This specifies how many times Charging Station must retry to download the firmware
                                                                                    // before giving up. If this field is not present, it is left to Charging Station to
                                                                                    // decide how many times it wants to retry. If the value is 0, it means: no retries.
    retryInterval?:                     types.  Seconds,                            // The interval in seconds after which a retry may be attempted. If this field is not
                                                                                    // present, it is left to Charging Station to decide how long to wait between attempts.
    requestId:                          types.  RequestId,                          // The Id of the request.
    firmeware:                          complex.Firmware,                           // Specifies the firmware to be updated on the Charging Station.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UpdateFirmwareResponse {
    status:                             types.  UpdateFirmwareStatus,               // Indicates whether the request was accepted.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}
...
export interface UsePriorityChargingRequest {
    transactionId:                      types.  TransactionId,                      // The transaction for which priority charging is requested.
    activate:                           boolean,                                    // True to request priority charging. False to request stopping priority charging.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface UsePriorityChargingResponse {
    status:                             types.  PriorityChargingStatus,             // Result of the request.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface VatNumberValidationRequest {
    vatNumber:                          types.  VATNumber,                          // VAT number to check.
    evseId?:                            types.  EVSEId,                             // EVSE id for which check is done
    customData?:                        complex.ICustomData                         // Customer specific data.
}

export interface VatNumberValidationResponse {
    vatNumber:                          types.  VATNumber,                          // VAT number that was requested.
    evseId?:                            types.  EVSEId,                             // EVSE id for which check was requested.
    status:                             types.  GenericStatus,                      // Result of operation.
    company?:                           complex.Address,                            // Company address associated with vatNumber.
    statusInfo?:                        complex.StatusInfo,                         // Detailed status information.
    customData?:                        complex.ICustomData                         // Customer specific data.
}
